(() => {
  ...
  // ============================================================
  // V4.13 — REACTIVE COMBAT & REINFORCEMENTS
  // Freeform combat is a continuous exchange: NPCs dodge, block,
  // counter, retreat and call allies without waiting for quick actions.
  // ============================================================
  ...
  function markCombatNpcDefeated(npcId,name,cause='combat'){
  ...
  function getOrCreateMarketMerchant(){
  ...
  function ensureNpcCombatIdentity(npc,preferredRole=null){
  ...
  function buildActionResolution(segments){const r=buildActionResolutionV415(segments);if(r?.has_action)r.integrity_outcome=buildIntegrityOutcome(r);return r;}
  function enforceFreeformActionResult(result,resolution){
  ...
  function performReinforcementAttack(ally,index,total){const n=ally.npcId?findNpcById(ally.npcId):null;if(n){const r=ensureNpcResourceState(n);if(ally.hp<ally.maxHp*.38&&r.healing>0){const heal=randInt(5,11);ally.hp=Math.min(ally.maxHp,ally.hp+heal);r.healing--;addLog(`<strong>${escapeHtml(ally.name)}</strong> uses a field dressing and recovers ${heal} HP.`,'ai-narration');return true;}if((ally.position==='wall'||ally.formationRole==='rear')&&/archer|crossbow/.test(String(ally.combatClass||ally.occupation).toLowerCase())){if(r.ammo<=0){ally.position='front';ally.formationRole='front';ally.combatClassKey='watchman';ally.combatClass=ally.combatClass==='Wall Archer'?'Disarmed Archer':ally.combatClass;ally.attacks=['slashes with a sidearm','jabs with a short blade'];addLog(`<strong>${escapeHtml(ally.name)}</strong> is out of ammunition and draws a sidearm.`,'system');}else r.ammo--;}if(/mage/.test(String(ally.combatClass||ally.occupation).toLowerCase())){if(r.mana<8){ally.attacks=['strikes with a staff'];}else r.mana-=8;}r.stamina=Math.max(0,r.stamina-6);}return performReinforcementAttackV415(ally,index,total);}
  ...
  function updateBattlefieldCohesion(delta,reason='battle'){const e=state.game.combat;if(!e)return;ensureTacticalBattleState();e.battlefield.cohesion=clamp(Number(e.battlefield.cohesion??78)+delta,0,100);e.battlefield.lastCohesionReason=reason;if(e.battlefield.cohesion<35&&!e.battlefield.broken){e.battlefield.broken=true;e.battlefield.formation='broken formation';e.battlefield.strategy='individual survival and improvised fighting after cohesion collapses';addLog(`<strong>Formation broken:</strong> enemy cohesion collapses.`,'event');}}
  ...
  function addNpcMemory(npc,text,meta={}){const r=addNpcMemoryV415(npc,text,meta);if(npc){if(meta.category==='help')npc.favourBalance=Number(npc.favourBalance||0)+1;if(meta.category==='hostility')npc.favourBalance=Number(npc.favourBalance||0)-1;updateNpcBond(npc);}return r;}
  ...
  // ---------- Turn/day wrappers ----------
  function advanceTurn(amount=1){advanceTurnV415(amount);updateNpcSchedules();updateLawCases();syncOwnershipLedger();updateCarryingState();}
  ...
  async function requestGeneratedArt(prompt,seed=1){
    ...
    try{
      const res=await fetch(cfg.endpoint,{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({kind:"image_generate",prompt:String(prompt).slice(0,1900),seed}),
        signal:controller.signal
      });
      const body=await res.json().catch(()=>({}));
      if(!res.ok||!body.dataURI)throw new Error(body.error||`Image generation failed (${res.status})`);
      return body.dataURI;
    }finally{clearTimeout(timer);}
  }

  // ------------------------------------------------------------
  // Questlines: simple and multi-stage
  // ------------------------------------------------------------
  const V418_QUESTLINES=[
    {
      key:"missing_caravan",title:"The Missing Caravan",
      desc:"A caravan failed to reach its destination and somebody needs to find out why.",
      stages:[
        {title:"Ask Around",objectiveKind:"urban_help",goal:1,description:"Gather a useful lead from locals."},
        {title:"Follow the Trail",objectiveKind:"explore",goal:2,description:"Search the road and surrounding ground for signs of the caravan."},
        {title:"Deal with the Threat",objectiveKind:"combat",goal:2,description:"Defeat the danger responsible for the disappearance."},
        {title:"Report Back",objectiveKind:"return",goal:1,description:"Return to the quest giver with what you learned."}
      ]
    },
    {
      key:"ruin_secret",title:"Secrets Beneath Old Stone",
      desc:"Strange activity around nearby ruins has attracted unwanted attention.",
      stages:[
        {title:"Reach the Ruins",objectiveKind:"travel",goal:1,description:"Travel out toward the suspected ruins."},
        {title:"Search the Site",objectiveKind:"explore",goal:2,description:"Investigate the ruin and identify what is happening."},
        {title:"Survive the Depths",objectiveKind:"combat",goal:1,description:"Overcome whatever is guarding the discovery."},
        {title:"Bring Back the Account",objectiveKind:"return",goal:1,description:"Return and explain what happened."}
      ]
    },
    {
      key:"local_problem",title:"A Problem Close to Home",
      desc:"A local resident needs practical help with a problem that has grown beyond them.",
      stages:[
        {title:"Understand the Problem",objectiveKind:"urban_help",goal:1,description:"Speak with those involved and understand the situation."},
        {title:"Take Action",objectiveKind:"combat",goal:1,description:"Resolve the dangerous part of the problem."},
        {title:"Return",objectiveKind:"return",goal:1,description:"Return to the person who asked for help."}
      ]
    }
  ];
  ...
  async function generateBestiaryImage(name){
    ...
    const img=document.querySelector(`[data-bestiary-img="${CSS.escape(name)}"]`),btn=document.querySelector(`[data-gen-bestiary="${CSS.escape(name)}"]`);
    ...
    try{
      ...
      await mediaCachePut(`bestiary:${name}`,art);if(img)img.src=art;
    }catch(err){showToast(String(err?.message||err));}
    finally{if(btn){btn.disabled=false;btn.textContent="Generate medieval illustration";}}
  }
  ...
  // ------------------------------------------------------------
  function npcPortraitPrompt(npc){
  ...
  async function generateNpcPortrait(npcId){
    ...
    const img=document.querySelector(`[data-npc-portrait="${CSS.escape(npcId)}"]`),btn=document.querySelector(`[data-gen-portrait="${CSS.escape(npcId)}"]`);
    ...
    try{
      ...
      await mediaCachePut(`npc:${npc.id}`,art);if(img)img.src=art;
    }catch(err){showToast(String(err?.message||err));}
    finally{if(btn){btn.disabled=false;btn.textContent="Generate portrait";}}
  }
  ...
  function peopleJournalCard(npc){
    ...
    return `<article class="people-card ${npc.dead?"dead":""}">
      <div class="npc-codex-layout">
        <div class="npc-portrait-column"><img class="npc-portrait" data-npc-portrait="${escapeHtml(npc.id)}" src="${placeholderArtSvg(npc.name,"npc")}" alt="${escapeHtml(npc.name)} portrait"><button class="ghost-button" data-gen-portrait="${escapeHtml(npc.id)}">Generate portrait</button><div class="npc-equipment-strip">${npcEquipmentStrip(npc)}</div></div>
  ...
  function createChildNpc(carrier,otherParentId){
  ...
  function advanceWorldDay(days=1){for(let i=0;i<days;i++){advanceWorldDayV417(1);advanceFamilySimulation();}}

  // ------------------------------------------------------------
  // V4.18 UI injection
  // ------------------------------------------------------------
  function injectAdventureUi(){
  ...
  function injectIntegrityUi(){injectIntegrityUiV417();injectAdventureUi();}
