(() => {
    ...
    screenRoot.innerHTML = `
      ...
      <div class="game-layout ${gameClass}">
        <aside class="side-stack left-panel">
          ...
          <section class="panel hud-card">
            <div class="hud-heading">Skills</div>
            <div class="inventory-list">
              ${Object.entries(c.skills || {}).length ? Object.entries(c.skills).map(([skill,level]) => `<div class="inventory-item"><span>${escapeHtml(skill)}</span><strong>${level}</strong></div>`).join("") : `<div class="inventory-item"><span>None</span></div>`}
            </div>
          </section>
        </aside>
    ...
    if (g.combat) {
      ...
      return `
        ...
        <div class="choice-grid">
          ...
          <button class="choice-button" data-action="combat-power"><strong>Use Technique</strong><small>${escapeHtml(state.character.signatureTechnique || CLASSES[state.character.className]?.skill || "Focused Strike")}</small></button>
  ...
  function combatAction(action) {
    ...
    if (action === "combat-power") {
      ...
      e.hp -= playerDamage;
      text = `You use <strong>${escapeHtml(c.signatureTechnique || CLASSES[c.className]?.skill || "Focused Strike")}</strong>, dealing <strong>${playerDamage} damage</strong>.`;
    }
