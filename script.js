(() => {
  ...
  function damageCombatTarget(targetNpcId,dmg,status=null,resolution=null){
  ...
  function applyReactiveDamageToPlayer(attacker,damage,move){
  ...
  function resolveReactivePlayerDefeat(cause='combat'){
  ...
  function formationDefenseBonus(targetId,actionText){
  ...
  function processCombatSupportRound(){
  ...
  async function submitRoleplayInput(prefill=null){const raw=String(prefill??document.getElementById('roleplayInput')?.value??'').trim();const npc=state.game.activeNpc;const tx=(state.world.transactions||[]).length;await submitRoleplayInputV413(prefill);if(npc&&raw)explicitEconomyFallback(raw,npc,tx);saveGame(false);renderGame();}
